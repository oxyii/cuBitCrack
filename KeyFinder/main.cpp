...
void resultCallback(KeySearchResult info)
{
    if (_config.resultsFile.length() != 0)
    {
        ...
        std::string wif = WIF::privateKeyToWIF(info.privateKey, info.compressed);
        std::string s = info.address + " " + info.privateKey.toString(16) + " " + wif + " " + info.publicKey.toString(info.compressed);
    ...
    if (_config.telegram)
    {
        _config.telegram->sendMessage(_config.telegram->keyFound(logStr));
    }
}
...
void usage()
{
    ...
    printf("--telegram TOKEN/CHAT   Send notifications to Telegram:\n");
    printf("                          BOT_TOKEN/CHAT_ID[/INSTANCE_NAME]\n");
    printf("                          where BOT_TOKEN is bot token, CHAT_ID is chat ID,\n");
    printf("                          and INSTANCE_NAME is optional instance identifier\n");
}
...
static KeySearchDevice *getDeviceContext(DeviceManager::DeviceInfo &device, unsigned int blocks, unsigned int threads, unsigned int pointsPerThread)
{
...
int run()
{
    ...
    try
    {
        ...
        if (_config.telegram)
        {
            _config.telegram->sendMessage(_config.telegram->messageEnd());
        }
    }
    catch (KeySearchException ex)
    {
        ...
        if (_config.telegram)
        {
            _config.telegram->sendMessage(_config.telegram->messageError(ex.msg));
        }
        return 1;
    }
...
int main(int argc, char **argv)
{
    ...
    for (unsigned int i = 0; i < args.size(); i++)
    {
        ...
        try
        {
            ...
            else if (optArg.equals("", "--telegram"))
            {
                std::string tgBotToken, tgChatId, tgInstanceName;

                // Parse telegram argument in format bot_token/chat_id[/instance_name]
                std::string telegramArg = optArg.arg;
                size_t firstSlash = telegramArg.find('/');
                if (firstSlash == std::string::npos)
                {
                    throw std::string("Invalid telegram format, expected bot_token/chat_id[/instance_name]");
                }
                tgBotToken = telegramArg.substr(0, firstSlash);

                size_t secondSlash = telegramArg.find('/', firstSlash + 1);
                if (secondSlash == std::string::npos)
                {
                    // No instance name provided
                    tgChatId = telegramArg.substr(firstSlash + 1);
                    tgInstanceName = "";
                }
                else
                {
                    // Instance name provided
                    tgChatId = telegramArg.substr(firstSlash + 1, secondSlash - firstSlash - 1);
                    tgInstanceName = telegramArg.substr(secondSlash + 1);
                }

                if (tgBotToken.empty() || tgChatId.empty())
                {
                    throw std::string("Bot token and chat ID cannot be empty");
                }

                _config.telegram = new Tg(tgBotToken, tgChatId, tgInstanceName);
            }
        }
        catch (std::string err)
        {
